...
import uz.gita.notesapp.utils.ExpandableCountryModel
...
class CountryStateExpandableAdapter(var context: Context, var countryStateModelList:MutableList<ExpandableCountryModel>) :  RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when(row.type){
            ExpandableCountryModel.PARENT -> {
                (holder as CountryStateParentViewHolder).countryName.text = row.countryParent.title
            ...
            ExpandableCountryModel.CHILD -> {
                (holder as CountryStateChildViewHolder).stateName.text = row.countryChild.title
                holder.capitalImage = row.countryChild.title.
            }
        }
    ...
    private fun expandRow(position: Int){
        ...
        when (row.type) {
            ExpandableCountryModel.PARENT -> {
                for(child in row.countryParent.bodies){
